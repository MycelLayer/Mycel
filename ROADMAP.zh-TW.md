...
2. 擴展 `M3` 的 reader-plus-governance 工作流程，但不要重新打開已經關閉的 minimal-client gate；既然最終獨立 dual-role 收尾已落地，接下來就把更廣的 governance persistence、更完整的 governance tooling、reader-facing profile ergonomics，以及仍屬 design draft 的 viewer / editor-maintainer / view-maintainer 三角色 checks-and-balances 方向明確保留下來

...
Status: draft, refreshed for a full planning-sync pass so the summary now reflects `M2` / `M3` / `M4` as the active planning lane; `M2` now records that the remaining closure work is no longer general nested/reparenting coverage but the still-coarse content-variant and metadata-variant competing-branch classification, `M3` still centers on broader governance persistence and reader-plus-governance follow-up beyond the current inspect/list/publish baseline, and `M4` records localhost transport proof, all three currently tracked production replication sub-proofs, and a first permanent messages-after-BYE session proof as landed while broader session/error-path interop remains open
...
| 10. Merge Generation | Partial | `M2` | replay verification and a conservative local merge-authoring profile exist, including structural move/reorder, new-parent reparenting, simple composed parent-chain coverage, a broader nested structural matrix, manual-curation smoke for nested parent-choice, nested sibling-choice, and composed-branch placement conflicts, richer direct/anchor-based competing placement reasons, landed metadata competing-variant handling for adopting or keeping primary over non-primary metadata additions, and an explicit manual-curation boundary for metadata removal because v0.1 patch ops do not yet express deletion; the main remaining `M2` gap is richer content-variant and metadata-variant competing-branch detail where multiple non-primary alternatives still collapse into coarse output |
| 11. CLI or API Surface | Partial | `M2` / `M3` / `M4` | verification, authoring, conservative merge authoring, editor-admission-aware reader inspection/render, governance inspect/list/publish, persisted governance index query surfaces, persisted governance relationship summaries in `view inspect` / `view list`, transcript-backed `sync pull`, and internal `sync peer-store` all exist, including optional snapshot/view flows, localhost multi-process proof, re-sync idempotency, depth-N catchup, partial-doc selective sync, and messages-after-BYE rejection; the remaining `M4` gap is broader session/error-path interop proof |

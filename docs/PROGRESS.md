...
| `M3` | Early partial | accepted-head reader workflows, bundle/store rendering, named fixed-profile reading, initial filtered/sorted `view` governance inspect/list/publish workflows | broader governance persistence, richer governance tooling, reader profile ergonomics |
...
| 11. CLI or API Surface | Partial | `M2` / `M3` | verification, authoring, reader inspection/render, and initial governance inspect/list/publish with filtered listing exist; sync remains open |

...
| `M3` | Early partial | accepted-head reader workflows, bundle/store rendering, named fixed-profile reading, minimal `view` governance inspect/list/publish workflows | broader governance persistence, richer governance tooling, reader profile ergonomics |
...
| 11. CLI or API Surface | Partial | `M2` / `M3` | verification, authoring, reader inspection/render, and minimal governance inspect/list/publish exist; sync remains open |

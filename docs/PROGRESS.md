...
3. keep `M4` narrow while peer-store sync proof grows toward the remaining session/error-path interop closure and the unfinished production replication sub-items
...
| `M4` | Early partial | wire envelope validation, `OBJECT` body verification, session reachability, store-backed bootstrap, peer-store-driven first-time / incremental sync proofs, capability-gated optional-message handling, localhost multi-process proof, and re-sync idempotency proof | remaining session/error-path interop coverage plus depth-N catchup and partial-doc selective sync |
...
| 11. CLI or API Surface | Partial | `M2` / `M3` / `M4` | verification, authoring, conservative merge authoring, editor-admission-aware reader inspection/render, governance inspect/list/publish, persisted governance index query surfaces, transcript-backed `sync pull`, and internal `sync peer-store` all exist, including optional snapshot/view flows and localhost multi-process proof; the remaining replication behavior is depth-N catchup and partial-doc selective sync |
| 12. Interop Test Minimum | Partial | `M1` / `M2` / `M4` | fixture isolation, reproducibility, stricter parser/replay smoke coverage, direct wire-envelope/signature/session tests, peer-store first-time / incremental sync proofs, optional-message coverage, localhost multi-process proof, and re-sync idempotency coverage exist, but the remaining session/error-path and production-replication cases are still open |

...
Status: draft, refreshed for a full planning-sync pass so the summary now reflects `M3` / `M4` as the active planning lane; `M2` replay/storage/rebuild closure is now landed at the current narrow scope after the richer mixed branch kinds, the multi-document rebuild-after-index-loss proof, and the richer metadata multi-variant rebuild/reporting proof all landed, `M3` still centers on broader governance persistence and reader-plus-governance follow-up beyond the current inspect/list/publish baseline, and `M4` records localhost transport proof, all three currently tracked production replication sub-proofs, and a first permanent messages-after-BYE session proof as landed while broader session/capability/error-path interop remains open

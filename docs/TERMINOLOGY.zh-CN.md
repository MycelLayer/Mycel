...
| governance | 治理 | 指依固定 profile，从已验证对象与治理信号中导出 accepted head 与 accepted reading 的规则系统。 |
...
| English term | 建议中文 | 补充说明 |
| --- | --- | --- |
| Code Quality Hotspots (CQH) | Code Quality Hotspots / 代码质量热点 | 指由 repo 脚本汇总出的代码风险热点报告；当前 issue `#31` 这类追踪项会用 `CQH` 作为缩写。 |

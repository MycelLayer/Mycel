...
- `sync web`：只同步 GitHub Pages。包括 `docs/progress.html` 与各语言 landing pages 中不属于 issue 入口的 HTML 摘要文案。
...
用 `docs/PROGRESS.md` 和 `docs/progress.html` 回答：
...
4. 如果公开摘要措辞发生变化，再更新 `docs/progress.html`
...
- GitHub Pages 上的 HTML 摘要面，比如 `docs/progress.html` 和各语言 landing pages 的非 issue 文案，在 `sync web` 到门槛时更新
...
3. `doc` 必须运行 `/workspaces/Mycel/scripts/check-plan-refresh.sh`
...
6. 如果 `sync web` 到门槛，再更新 `docs/progress.html` 和各语言 landing pages 的非 issue HTML 摘要
...
2. 除非当前 mailbox 明确指向 archive 里仍未解决的条目，否则不要回头扫描 archived mailboxes
3. `doc` 必须运行 `/workspaces/Mycel/scripts/check-plan-refresh.sh`
4. 更新 `docs/progress.html` 和各语言 landing pages 的非 issue HTML 摘要

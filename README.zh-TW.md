...
- 可驗證的簽章治理訊號
- 多個有效分支並存，而不要求全域強制共識
- 由固定 profile 規則導出的預設閱讀版本，而不是任意本地偏好
...
- 可驗證的修訂歷史：revision 預期要能被 replay 與檢查，而不是只靠信任。
- 依規則導出的預設閱讀版本：預設採用的 accepted head 來自固定 profile 規則與已驗證的 View objects。
- 容許分叉：多個 head 可以並存，而不假設整個網路必須只有一個全域真相。
- 中立的協定核心：領域語義應放在 profiles 與 app layers，不應寫死進 core protocol。

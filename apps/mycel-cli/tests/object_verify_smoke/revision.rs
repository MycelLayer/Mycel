...
#[test]
fn object_verify_json_fails_for_revision_with_parent_dependency_id_mismatch() {
    let dir = create_temp_dir("object-verify-revision-parent-id-mismatch");
    let parent_revision_path = dir.path().join("revision-parent.json");
    let patch_path = dir.path().join("patch.json");
    let revision_path = dir.path().join("revision.json");

    let mut parent_revision = signed_object(
        json!({
            "type": "revision",
            "version": "mycel/0.1",
            "doc_id": "doc:test",
    ...
    parent_revision["revision_id"] = Value::String("rev:wrong-parent".to_string());
    ...
    let patch = signed_object(
        json!({
            ...
            "version": "mycel/0.1",
            "doc_id": "doc:test",
            "base_revision": "rev:wrong-parent",
            "timestamp": 1777778889u64,
            "ops": []
        }),
        "author",
    ...
    assert!(
        json["errors"]
            ...
            .is_some_and(|errors| errors.iter().any(|entry| {
                entry.as_str().is_some_and(|message| {
                    message.contains("failed to verify parent revision 'rev:wrong-parent'")
                        && message.contains(
                            "declared revision_id does not match recomputed canonical object ID",
                        )
                })
            })),
        "expected parent dependency ID mismatch error, stdout: {}",
        stdout_text(&output)
    );
}

#[test]
fn object_verify_json_fails_for_revision_with_parent_dependency_bad_signature() {
    let dir = create_temp_dir("object-verify-revision-parent-bad-signature");
    let parent_revision_path = dir.path().join("revision-parent.json");
    let patch_path = dir.path().join("patch.json");
    let revision_path = dir.path().join("revision.json");
    ...
    let parent_revision_id = parent_revision["revision_id"]
        ...
        .to_string();
    parent_revision["signature"] = Value::String("sig:ed25519:AA==".to_string());
    fs::write(
        &parent_revision_path,
        serde_json::to_string_pretty(&parent_revision)
            .expect("parent revision JSON should serialize"),
    ...
    let patch = signed_object(
        json!({
            ...
            "version": "mycel/0.1",
            "doc_id": "doc:test",
            "base_revision": parent_revision_id.clone(),
            "timestamp": 1777778889u64,
            "ops": []
        }),
        "author",
            ...
            .is_some_and(|errors| errors.iter().any(|entry| {
                entry.as_str().is_some_and(|message| {
                    message.contains(&format!(
                        "failed to verify parent revision '{parent_revision_id}'"
                    )) && message.contains("invalid Ed25519 signature bytes")
                })
            })),
        "expected parent dependency signature error, stdout: {}",
        stdout_text(&output)
    );
}

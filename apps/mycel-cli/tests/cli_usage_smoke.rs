...
use common::{
    ...
    assert_view_diff_help, assert_view_document_help, assert_view_history_help,
...
#[test]
fn view_history_help_prints_structured_clap_help() {
    let assert = mycel_command(&["view", "history", "--help"])
        .assert()
        .success();
    let stdout = assert_stdout_text(&assert);

    assert_eq!(assert_stderr_text(&assert), "");
    assert_view_history_help(&stdout);
}

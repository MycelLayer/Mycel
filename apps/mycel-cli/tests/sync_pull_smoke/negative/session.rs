...
#[test]
fn sync_pull_json_rejects_object_before_hello() {
    let signing_key = signing_key();
    let sender = "node:alpha";
    ...
    let transcript_dir = create_temp_dir("sync-pull-object-before-hello");
    let transcript_path = transcript_dir
        .path()
        .join("object-before-hello-transcript.json");
    let store_root = create_temp_dir("sync-pull-object-before-hello-store");
    write_transcript(
        &transcript_path,
        &json!({
            "peer": {
    ...
    assert!(
        json["errors"]
            ...
            .is_some_and(|errors| errors.iter().any(|error| {
                error.as_str().is_some_and(|message| {
                    message.contains("wire OBJECT requires prior HELLO from 'node:alpha'")
                })
            })),
        "expected prior-HELLO OBJECT error, stdout: {}",
        stdout_text(&output)
    );
    assert!(!store_root
        .path()
...
#[test]
fn sync_pull_json_rejects_want_before_manifest() {
    let signing_key = signing_key();
    let sender = "node:alpha";
    ...
    assert!(
        json["errors"]
            ...
            .is_some_and(|errors| errors.iter().any(|error| {
                error.as_str().is_some_and(|message| {
                    message.contains("wire WANT requires prior MANIFEST or HEADS from 'node:alpha'")
                })
            })),
        "expected WANT-before-manifest error, stdout: {}",
        stdout_text(&output)
    );
    assert!(!store_root
        .path()
...
fn sync_pull_json_rejects_unadvertised_root_object_after_root_want() {
    ...
    let unadvertised_root_id = unadvertised_root["payload"]["object_id"]
        .as_str()
        .expect("unadvertised root revision object id should exist")
        .to_string();
    let transcript_dir = create_temp_dir("sync-pull-unadvertised-root-object");
    let transcript_path = transcript_dir
        .path()
        .join("unadvertised-root-object-transcript.json");
    let store_root = create_temp_dir("sync-pull-unadvertised-root-object-store");
    write_transcript(
        &transcript_path,
        &json!({
            "peer": {
    ...
    assert!(
        json["errors"]
            ...
            .is_some_and(|errors| errors.iter().any(|error| {
                error.as_str().is_some_and(|message| {
                    message.contains(&format!(
                        "wire OBJECT '{unadvertised_root_id}' was not requested from '{sender}'"
                    ))
                })
            })),
        "expected unadvertised root OBJECT error, stdout: {}",
        stdout_text(&output)
    );
    assert!(!store_root
        .path()

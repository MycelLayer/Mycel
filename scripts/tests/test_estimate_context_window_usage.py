...
class EstimateContextWindowUsageCliTest(unittest.TestCase):
    ...
    def test_named_calibration_shortcut_applies_coding_bootstrap_sample(self) -> None:
        spec = {
            "context_window": 258000,
            "current_input_tokens": 70000,
            "last_output_tokens": 1200,
        ...
        self.assertIn("估計 token 使用量：104K / 258K。", proc.stdout)
        self.assertIn("Raw estimate before calibration: 71,200", proc.stdout)
        self.assertIn("additive calibration (+33,000 tokens) from fixed delta", proc.stdout)

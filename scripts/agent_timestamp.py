...
def build_message(stage: str, *, agent: str | None, scope: str | None, now: datetime | None = None) -> str:
...
def main() -> int:
    args = parse_args()
    print(build_message(args.stage, agent=args.agent, scope=args.scope, now=parse_now(args.now)))
    return 0

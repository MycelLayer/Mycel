...
- If `.agent-local/dev-setup-status.md` says `Status: ready`, treat that as static workspace readiness only; before running tests, scripts, `cargo run`, or `cargo test`, do a lightweight runtime preflight such as `command -v cargo bash rg` plus any shell utilities used in the exact command (for example `grep` or `tail`). If a required command is missing or a command exits `126`/`127`, report it as an environment block first and do not treat the result as product validation or checklist closure. <!-- item-id: workflow.runtime-preflight-before-verification -->

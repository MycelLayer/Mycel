...
#[test]
pub(super) fn revision_replay_reports_multi_hop_patch_wrong_type_with_ancestry_context() {
    let (signing_key, public_key) = signer_material();
    let mut parent_revision = json!({
        "type": "revision",
        "version": "mycel/0.1",
    ...
    assert!(
        summary.errors.iter().any(|message| {
            message.contains(&format!(
                ...
                .contains("patch 'patch:wrong-patch' is a 'revision' object instead of 'patch'")
        }),
        "expected nested ancestry context error, got {summary:?}"
    );
}
